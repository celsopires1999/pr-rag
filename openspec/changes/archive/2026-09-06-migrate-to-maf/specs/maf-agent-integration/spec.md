...
The system SHALL register an `AIAgent` instance via dependency injection, wrapping the existing `IChatClient` with the `AsAIAgent()` extension method, and SHALL inject this agent into `ChatService` for chat request processing.

#### Scenario: Agent created from existing chat client
- **WHEN** the application starts and DI is configured
- **THEN** the system creates an `AIAgent` by wrapping the registered `IChatClient` via `AsAIAgent()`, with the agent's instructions derived from the existing system prompt

#### Scenario: Agent injected into chat service
- **WHEN** `ChatService` is constructed
- **THEN** it receives the `AIAgent` instance (or a wrapper around it) instead of calling `IChatClient.GetResponseAsync` directly

### Requirement: MAF tool registration
The system SHALL register the four existing tool functions (`search_by_codes`, `search_semantic`, `activate_skill`, `create_requisition`) with the `AIAgent` via its `Tools` property, using the same `AIFunctionFactory.Create` delegates already in use.

#### Scenario: Tools bound to agent
- **WHEN** the `AIAgent` is configured in DI
- **THEN** it exposes the four tool functions so that `RunAsync` can invoke them during the agentic reasoning loop

#### Scenario: Tool implementations unchanged
- **WHEN** a tool is invoked during a chat request
- **THEN** the same repository, embedding, skill, and requisition-writer methods are called with the same parameters and return the same results as before
