...
The four available tools SHALL be registered on the `AIAgent` via `AIFunctionFactory`, mirroring the existing repository retrieval methods, so a tool call maps to a single PostgreSQL query.
...
The system SHALL build the final answer using the requisition context that the model actually retrieved through its tool calls during `AIAgent.RunAsync()`, rather than unrelated context.
...
The system SHALL keep `top_k` and `min_similarity` configurable and SHALL apply them to MAF agent-driven retrieval, with environment-provided defaults when the client omits them.
...
- **WHEN** the agent produces an answer without retrieved requisition context
