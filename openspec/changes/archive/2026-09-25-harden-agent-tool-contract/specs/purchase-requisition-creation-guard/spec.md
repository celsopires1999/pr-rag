...
- **THEN** the system persists the requisition in the dedicated Postgres schema and returns a result with the success flag true and the created requisition id
...
- **THEN** the system does not persist anything and returns a result with the success flag false and a message stating the supplier is not registered for that item, so no requisition was created
