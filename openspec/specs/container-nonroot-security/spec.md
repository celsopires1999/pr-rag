# Container Non-Root Security

## Purpose

Run every service container's primary workload process as an unprivileged, non-root user rather than as `root`, so that a compromise of any workload does not carry full privileges inside its container. This capability covers the runtime containers (API, web, data generator, integration tests, database, and the DevContainer) and preserves the writable bind mounts the unprivileged users rely on.
