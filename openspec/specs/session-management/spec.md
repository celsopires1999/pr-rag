...
Provide lightweight session management for the chat GUI so the user can see which conversation context is active and can discard the current session to start fresh, backed by a server-side discard endpoint.
